...
public struct MatrixX<T>: TensorProtocol
{    
    ...
    /// Determine whether to show name when displaying matrix.
    public var showName: Bool

    /// Formatter to be used in displaying tensor elements.
    public var format: NumberFormatter

    /// Matrix-only accessors
    public var rows: Int { return self.size[0] }
    public var columns: Int { return self.size[1] }
    ...
    ///    - showName: determine whether to print the matrix name; defaults to true if the matrix is
    ///        given a name, otherwise to false
    public init(_ matrix: MatrixX<T>, name: String? = nil, showName: Bool? = nil)
    {
        self.init(matrix.size, matrix.data, name: name, showName: showName)
    ...
    ///    - index: linear index range 
    /// - Returns: new matrix composed of slice
    public subscript(_ index: SliceIndex) -> MatrixX<T>
    {
        get
        {
            let range = _convertToCountableClosedRange(index)
            ...
            return MatrixX([Array(self.data[range])], name: sliceName, showName: self.showName)
        }
        set(newVal)
        {
            let range = _convertToCountableClosedRange(index)
    ...
    ///    - columns: column range
    /// - Returns: new matrix composed of slice
    public subscript(_ rows: SliceIndex, _ columns: SliceIndex) -> MatrixX<T>
    {
        get 
        { 
            let ranges = _convertToCountableClosedRanges([rows, columns])
            ...
            return MatrixX(newSize, newData, name: sliceName, showName: self.showName) 
        }
